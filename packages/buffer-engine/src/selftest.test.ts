...
describe('runSiteBufferSelfTest day-window timezone offset', () => {
  it('uses a BST (+01:00) offset for yesterday’s summer window', async () => {
    await runSiteBufferSelfTest(makeAdapter(makeKV()), {
      now: new Date('2026-06-28T05:00:00Z'),
    });
    const { start, end } = dueAtWindow();
    expect(start).toBe('2026-06-27T00:00:00+01:00');
    expect(end).toBe('2026-06-28T00:00:00+01:00');
  });

  it('uses a GMT (+00:00) offset for yesterday’s winter window', async () => {
    await runSiteBufferSelfTest(makeAdapter(makeKV()), {
      now: new Date('2026-01-15T05:00:00Z'),
    });
    const { start, end } = dueAtWindow();
    expect(start).toBe('2026-01-14T00:00:00+00:00');
    expect(end).toBe('2026-01-15T00:00:00+00:00');
  });
});
