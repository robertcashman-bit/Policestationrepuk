...
export const DEFAULT_PRODUCTION_KICK_STEPS: KickStep[] = [
  ...
  // Operator-only Resend probe for RepUK (PSA agent-cover permanently disabled).
  {
    path: '/api/cron/firm-outreach-probe',
    label: 'Pre-flight email probe (policestationrepuk)',
  },
  // Flush FIRST while cooldown override is live — do not let requalify/enrich burn the window.
  {
    ...
    label: 'Dry-run preview (RepUK campaign, safe send limit)',
    optional: true,
  },
  {
    path: '/api/cron/firm-outreach-send?limit=150',
    label: 'Send flush 1 (whatsapp_invite_v1)',
  },
  ...
  // PSA agent-cover seed/enrich intentionally omitted — permanently disabled.
