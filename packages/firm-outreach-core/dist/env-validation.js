...
function validateOutreachEnv(opts) {
    ...
    if (!digest) {
        errors.push('FIRM_OUTREACH_DIGEST_EMAIL (or BUFFER_SCHEDULER_NOTIFY_EMAIL / OWNER_EMAIL / ADMIN_EMAILS) missing');
    }
    ...
    if (!process.env.FIRM_OUTREACH_FROM_EMAIL?.trim()) {
        warnings.push('FIRM_OUTREACH_FROM_EMAIL not set — using verified default PoliceStationRepUK <noreply@policestationrepuk.org>');
    }
