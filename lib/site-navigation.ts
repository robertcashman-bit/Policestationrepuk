...
/** Emit lowercase public hrefs for known PascalCase App Router folders. */
function withPublicHref<T extends { href: string }>(link: T): T {
  return { ...link, href: publicPath(link.href) };
}

function withPublicHrefs<T extends { href: string }>(links: readonly T[]): T[] {
  return links.map(withPublicHref);
}

function footerLinksToNav(links: readonly FooterLink[]): HeaderNavLink[] {
  return links.map(({ href, label, external }) => ({
    href,
    text: label,
    external,
  }));
...
export const HEADER_NAV_BLOG_LINKS: HeaderNavLink[] = withPublicHrefs([
...
export const WHATSAPP_PAGE_REPS = publicPath("/WhatsApp/reps");
export const WHATSAPP_PAGE_SOLICITORS = publicPath("/WhatsApp/solicitors");
export const WHATSAPP_PAGE_FIRMS = publicPath("/WhatsApp/firms");
...
export const HEADER_HELP_HREF = publicPath("/FAQ");
export const HEADER_LOGIN_HREF = publicPath("/Account");
...
export const FOOTER_DIRECTORIES: FooterLink[] = withPublicHrefs([
  ...
  { href: "/find-station", label: "Stations" },
...
export const FOOTER_FOR_REPRESENTATIVES: FooterLink[] = withPublicHrefs([
...
export const HEADER_NAV_GUIDES: HeaderNavLink[] = withPublicHrefs([
  ...
  { href: "/police-station-rights-uk", text: "Police station rights UK" },
  {
    ...
    text: "Free legal advice (police station)",
  },
]);
...
export const FOOTER_COMMUNITY: FooterLink[] = withPublicHrefs([
...
export const FOOTER_LEGAL: FooterLink[] = withPublicHrefs([
...
export const HEADER_NAV_MORE: HeaderNavLink[] = dedupeNavLinks(
  withPublicHrefs([
    ...
    {
      href: "/police-station-representative",
      text: "Police station representative",
    },
    {
      href: "/criminal-solicitor-police-station",
      text: "Criminal solicitor — police station",
    },
...
export const FOOTER_GUIDES: FooterLink[] = HEADER_NAV_GUIDES.map(
  ({ href, text, external }) => ({
    href,
...
export const FOOTER_FEES_FORMS: FooterLink[] = HEADER_NAV_FEES_FORMS.map(
  ({ href, text, external }) => ({
    href,
    label: text,
    external,
...
export const FOOTER_PARTNERS: FooterLink[] = withPublicHrefs([
  ...
  {
    href: PSRTRAIN_TRAINING_HREF,
    label: "PSR Train (PSRAS prep)",
    external: true,
  },
  { href: "/directory/kent", label: "Kent police station reps" },
  {
    href: "/police-station-representative",
    label: "Police station representative",
  },
  {
    href: "/criminal-solicitor-police-station",
    label: "Criminal solicitor — police station",
  },
...
export const HEADER_NAV_DROPDOWNS: HeaderNavDropdown[] = [
  {
    ...
    labelHref: publicPath("/Blog#all-posts"),
  },
  {
    label: "For Reps",
